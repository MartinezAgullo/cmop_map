...
// MASCAL is skewed: more T3 than T2 and more T2 than T1, with a few expectant (T4) and a
// few dead; more role-1
...
const DEFAULT_DISTRIBUTIONS = {
  triage:        { GREEN: 45, YELLOW: 30, RED: 15, BLUE: 5, BLACK: 5 },  // T3 > T2 > T1, 5 % T4, 5 % KIA
  vehicleRole:   { 1: 4, 2: 3, 3: 2, 4: 1 },                    // triangular, as in the notebook
  facilityRole:  { 1: 4, 2: 3, 3: 2, 4: 1 },                    // R1 > R2 > R3 > R4
  vehicleMedium: { ground: 4, air: 1 },                         // 80 % ground, 20 % air
...
const INJURIES = {
  ...
  // T4 Expectant: alive, but not survivable with the care a MASCAL can spare them.
  BLUE: ['Full-thickness burns 80% TBSA', 'Open penetrating head injury, GCS 3',
         'Bilateral above-knee amputation in profound shock', 'Massive thoraco-abdominal crush'],
};
...
function generateMascalScenario(options = {}) {
  ...
  const meta = {
    ...
    description: `Random MASCAL around ${centre.label}: ${nCasualties} casualties `
      + `(${['RED', 'YELLOW', 'GREEN', 'BLUE', 'BLACK'].map(t => `${triageMix[t] || 0} ${t}`).join(', ')}), `

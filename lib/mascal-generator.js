...
// MASCAL is skewed: more T3 than T2 and more T2 than T1, more role-1 vehicles than role-2,
// more role-1 facilities than role-2 and so on up the roles, and only about one ambulance
// in three able to lift more than one casualty per cycle.
// ---------------------------------------------------------------------------
...
const DEFAULT_DISTRIBUTIONS = {
  triage:        { GREEN: 0.5, YELLOW: 0.3, RED: 0.2 },        // T3 > T2 > T1
  vehicleRole:   { 1: 0.65, 2: 0.35 },                          // medevac_role_1 > role_2
  facilityRole:  { 1: 0.4, 2: 0.3, 3: 0.2, 4: 0.1 },            // R1 > R2 > R3 > R4
  vehicleMedium: { ground: 0.8, air: 0.2 },
...
function generateMascalScenario(options = {}) {
  ...
  // Facilities: the rear of the area, in the outer half of the radius.
  const facilityRoles = apportion(random, dist.facilityRole, nFacilities).map(Number);
  ...
  triages.forEach((triage, i) => {
    ...
    entities.push({
      nombre: `${prefix}-CAS-${i + 1} (WIA)`,
      descripcion: `WIA - ${injury}`,
    ...
    medicalDetails.push({
      ...
      vital_signs: [{ ...(VITALS[triage] || VITALS.GREEN)(), recorded_at: recordedAt }],
  ...
  if (triages.includes('RED')) {
    ...
    if (!vehicles.some(v => v.tipo_elemento === 'medevac_role_2')) {
  ...
  const meta = {
    ...
    description: `Random MASCAL around ${centre.label}: ${nCasualties} casualties `
      + `(${['RED', 'YELLOW', 'GREEN'].map(t => `${triageMix[t] || 0} ${t}`).join(', ')}), `
